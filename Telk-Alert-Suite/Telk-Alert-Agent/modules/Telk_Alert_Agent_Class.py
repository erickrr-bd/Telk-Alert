...
class TelkAlertAgent:
	...
	def start_telk_alert_agent(self) -> None:
		...
		try:
			...
			self.logger.create_log("Telk-Alert-Agent v4.0 - March 2025", 2, "_start", use_stream_handler = True)
			if path.exists(self.constants.TELK_ALERT_AGENT_CONFIGURATION):
				...
				while True:
					...
					if telk_alert_status == "Not running":
						...
						alert_rules_total = len(self.utils.get_yaml_files_in_folder(self.constants.ALERT_RULES_FOLDER))
						if not (telk_alert_threads - 1) == alert_rules_total:	
							telegram_message = self.generate_telegram_message_threads(telk_alert_threads, alert_rules_total)
							response_http_code = self.telegram.send_telegram_message(telegram_bot_token, telegram_chat_id, telegram_message)
							self.create_log_by_telegram_code(response_http_code)
						self.logger.create_log(f"Threads: {telk_alert_threads - 1}, Alert Rules: {alert_rules_total}", "_statusService", use_stream_handler = True)
	...
	def generate_telegram_message_threads(self, total_threads: int, alert_rules_total: int) -> str:
		"""
		...
		Parameters:
			...
			alert_rules_total (int): Total alert rules created.

		Returns:
			telegram_message (str): Message to be sent via Telegram.
		"""
		telegram_message = "" + u'\u26A0\uFE0F' + " Telk-Alert Service " + u'\u26A0\uFE0F' + "\n\n" + u'\u23F0' + "Service Status Validation Time: " + strftime("%c") + "\n\n\n"
		telegram_message += "" + u'\U0001f4cb' + " The Telk-Alert service has restarted because there are " + str(total_threads) + '/' + str(alert_rules_total) + " alert rules running.\n\n"
		return telegram_message
