	...
	def startTelkAlertAgent(self):
		...
		try:
			...
			if path.exists(self.__constants.PATH_TELK_ALERT_AGENT_CONFIGURATION_FILE):
				self.__logger.generateApplicationLog("Configuration file found in: " + self.__constants.PATH_TELK_ALERT_AGENT_CONFIGURATION_FILE, 1, "__readConfigurationFile", use_stream_handler = True)
			...
			self.__logger.generateApplicationLog("File Not Found or OS Error. For more information, see the logs.", 3, "__start", use_stream_handler = True)
